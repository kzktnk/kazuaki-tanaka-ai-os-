...
- 「何を書くべきか」の構造であり、機能要件の充足そのものは判断しない。ドメイン知識は別途。  
- 発注者に目的・優先・対象組織を決める権限と準備がないと、「RFP で要件を決めてもらう」状態を防げない。  
- パッケージ選定が調達前に必要な場合は、製品選定と組み合わせる。  
- 業界規制がある場合はコンプライアンス要件を足す。  
- 実行順序は Playbook。法定の公共調達手続は扱わない。  

## Risks

- 未確定を確定要件として書き、後の変更がすべて追加コストになる。  
- 発注者が決めるべきことまでベンダーに委ね、比較軸が消える。  
- 評価基準を配布後に作り、印象採点になる。  
- 変動要因が浅く、提案が早期に陳腐化する。  
- 特定ベンダー前提の構造になり、競争が形骸化する。  
- 移行・データ・運用を暗黙に含め、見積が割れない。  

## Examples

Weak scope: 「対象システムを刷新する。」  
Better: 組織・業務（含む／含まない）・データ移行・IF・運用を軸ごとに明示し、移行・教育を暗黙に含めていないか確認する。  
Purpose / estimate の Weak–Better は本文各節。実名事例は載せない。

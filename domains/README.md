# Domains

Reusable industry and functional context. Generalized only — no confidential client content.

See `ARCHITECTURE.md` §6.3.

| File | Scope |
|------|--------|
| `energy-utilities.md` | Parent: generation / T&D (stub) / retail-customer, plus Data-AI and operating-model principles |
| `public-defense.md` | Parent: Japan public-sector / defense IT — buyer vs seller, procurement lifecycle, 保全 |

Future splits (when a section outgrows the parent): `energy-utilities/asset-management.md`, `retail-customer.md`, `digital-ai.md`.

...
def build_why_slide(prs, slide_no):
    ...
    add_round_box(
        ...
        [
            ("いま起きていること", True),
            "・4月より発電所長が経営責任者に。DX戦略・AMPでも発電所主体を目指している",
            "・一方で「何をどこまで任せるか」がまだ決まっていない",
            "・AI検証（③-a）の議論が進んでも、自立運営の前提が共有されていない",
        ],
    )
    add_round_box(
        slide,
...
def build_what_slide(prs, slide_no):
    ...
    add_bullets(
        ...
        Inches(12.33),
        Inches(0.45),
        ["※ ③-a（AI検証）を進める前提として、まず③-bの定義を固める必要がある"],
        9,
    )
...
def build_positioning_slide(prs, slide_no):
    slide = prs.slides.add_slide(prs.slide_layouts[0])
    set_bg(slide)
    add_section_title(slide, "3. DX戦略 × AMP構築PJ × モデルプラント（位置付け）")
    add_key_message(
        slide,
...
def build_next_slide(prs, slide_no):
    ...
    data = [
        ["時期", "アクション", "成果物・ねらい"],
        ["8/7–8/12", "PO・プロジェクトオーナーとのすり合わせ", "定義・論点のたたき台合意"],
        ["8/13", "経営層ディスカッション", "定義合意 ＋ How論点4つの優先順位"],
        ["9月以降", "③-b具体設計", "拠点選定・権限/KPI設計・体制案・スケジュール"],

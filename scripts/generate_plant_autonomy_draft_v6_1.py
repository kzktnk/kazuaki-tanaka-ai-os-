...
def build_purpose_slide(prs, slide_no):
    ...
    add_round_box(
        ...
        [
            ...
            "・経営層ディスカッションのねらい：IBM仮説へのフィードバック ＋ 方策の優先順位合意",
        ],
        10,
    )
...
def build_fact_slide(prs, slide_no):
    ...
    data = [
        ...
        [
            "組織・責任",
            "4月より発電所長が経営責任者に。本社経営統括から数値目標を受領",
            "ヒアリング項目セット",
        ],
        [
            "AMプログラム",
            "目指す姿＝発電所自立運営でユニット価値最大化。"
        ...
        [
            "推進体制",
            "クライアント推進体制案：発電所長＝意見提示・整合判断・先行検証。"
            "モデルPL＝AI導入・技術検証が中心",
            "推進体制案（推進体制案）",
        ],
        [
            "モデルPL",
            "トラックA（AI検証）のみ計画。UC候補・拠点選定は未着手。"
            "「自律運営PLではない」と明言",
            "追加ヒアリング項目\nクライアント推進体制案",
        ],
        [
    ...
    add_source_note(slide, "※ 上表はIBMが初回ヒアリングおよびクライアントご共有資料から整理した事実。解釈・仮説は次スライド以降")
...
def build_issue_slide(prs, slide_no):
    ...
    data = [
        ...
        [
            ...
            "4月移行済み vs 推進体制案上の発電所長役割が限定的\n初の試みで現場・本社とも不慣れ",
        ],
        [
            "2",
            "KPI管理（本社）と実行権限（現場）の非対称",
            "推進体制案KPI管理機能：KPI主担=発電所 vs 本社集中方針の矛盾",
        ],
        [
            "3",
            "モデルPL＝AI技術検証に限定\n自立経営の運用モデル実証場がない",
            "追加ヒアリング／クライアント推進体制案のモデルPL定義",
        ],
        [
            "4",
            "DX推進体制とアセット管理構築プログラムが別体系\n（合流前提だが接続設計なし）",
            "推進体制案／AMプログラム群／DX戦略が並走",
        ],
        [
            "5",
            "「困らないサポート」が体制に未制度化",
            "AMプログラム要件セットの要請 vs 推進体制案にエスカレーション未記載",
        ],
    ]
...
def build_hypothesis_slide(prs, slide_no):
    ...
    add_key_message(
        slide,
        "発電所長が本社KPI統括/目標のもと、権限の範囲内で収支・運営・改善を自ら回す"
    ...
    lines = [
        ...
        "発電所自立経営とは——",
        "",
        "① 本社KPI統括/目標のもと、発電所長が「社長のような役割」で",
        "　 収支・運営・改善を現場で判断・実行する",
        "",
        "② ガバナンスは成熟度に応じて設計する",
    ...
    add_round_box(
        ...
        [
            ("クライアント既存構想との接続", True),
            "・AMプログラム：発電所主体の収支計画PDCA（要件セット）",
            "・DX戦略：自立経営領域の実現手段（DX戦略資料）",
            "・運営効率化 計画項目：自律的AM・迅速意思決定（デジタル推進 年間計画）",
            "",
            ("≠ 以下ではない", True),
            "・一度きりの権限委譲／AI導入だけ／トラックA技術PoC",
            "",
            ("＝ 以下である", True),
    ...
    add_bullets(
        ...
        Inches(1.15),
        [
            ("トラックB 自立運営モデル発電所 ／ DX・AIの位置づけ", True),
            "・トラックB：上記運用モデル（成熟度×権限×PDCA）を1拠点で試す場",
            "",
            ("DX・AIの3層（IBM仮説）", True),
    ...
    add_source_note(slide, "根拠：ヒアリング項目セット、DX戦略資料、クライアント推進体制案、デジタル推進 年間計画（ナレッジ形式知化）", 6.05)
...
def build_tobe_slide(prs, slide_no):
    ...
    data = [
        ...
        [
            "KPI",
            "設定（本社）と主担（推進体制案KPI管理機能）が混在",
            "KPI設定＝本社経営統括／達成責任＝発電所長／横串管理＝PMO",
        ],
        [
            "DX・AI",
            "AI検証＝技術・効率化中心",
    ...
    add_round_box(
        ...
        [
            ...
            "Level 2以降：実証データに基づき段階拡大（トラックBで検証）",
        ],
        9,
    )
...
def build_approach_slide(prs, slide_no):
    ...
    add_key_message(
        slide,
        "定義合意 → トラックBで成熟度×権限×PDCAを実証 → 制度化。"
        "権限委譲は「どこから・どこまで・いつ広げるか」をトラックB設計で決める",
    )
    steps = [
        ("Step 1｜定義合意", "第2回すり合わせ–経営層ディスカッション", "IBM仮説（Slide 3）への合意・修正"),
        (
            "Step 2｜トラックB設計",
            "8月下旬–9月",
...
def build_positioning_slide(prs, slide_no):
    ...
    add_key_message(slide, "DX・AMプログラム・モデルPLは別PJではなく、IBM仮説（運営モデル変革）の異なるレイヤー")
    data = [
        ["取組", "レイヤー", "IBM仮説との関係"],
        ["DX戦略", "変革テーマ", "自立経営の手段＋効率化＋高度化（3層）"],
        ["アセット管理構築プログラム", "経営プロセス", "発電所主体PDCA＝自立経営の中核プロセス"],
        ["トラックA（AI検証）", "技術検証", "トラックBの入力。単独では自立経営にならない"],
        ["トラックB 自立運営", "運営モデル実証", "IBM仮説そのものを1拠点で試す"],
    ]
...
def build_prove_slide(prs, slide_no):
    slide = prs.slides.add_slide(prs.slide_layouts[0])
    set_bg(slide)
    add_section_title(slide, "7. トラックBで何を実証するか")
    add_key_message(
        slide,
    ...
    data = [
        ...
        ["3", "収支PDCA（AMプログラム接続）", "AMプログラム今年度ゴールの先行実証"],
    ...
    add_round_box(
        ...
        [
            ...
            "・AI題材：効率化（短期効果施策等）＋高度化（ナレッジ形式知化・デジタル推進 年間計画）",
            "・トラックA成果はトラックBの「手段・高度化」層に接続",
        ],
        9,
    )
...
def build_next_slide(prs, slide_no):
    ...
    data = [
        ["時期", "アクション", "ねらい"],
        ["すり合わせ期間", "PO・プロジェクトオーナーですり合わせ", "Fact/Issue/IBM仮説へのフィードバック"],
        ["経営層ディスカッション", "経営層ディスカッション", "IBM仮説合意 ＋ Approach優先順位"],
        ["9月以降", "トラックB具体設計", "権限/KPI/拠点/題材/体制案"],
        ["来週", "エグゼクティブサマリー", "本編確定後に別途"],
    ]
    add_table(slide, data, Inches(0.50), Inches(2.60), Inches(12.33), Inches(2.35), 10)
    add_source_note(slide, "参照：初回ヒアリングシート、クライアント推進体制案、DX戦略資料、事前ご案内資料、アセット管理構築プログラム、デジタル推進 年間計画", 5.20)
